...
Deno.test("Router handles different HTTP methods", () => {
  ...
  // Test GET
  const getMatch = testRouter.match(
    createRequest("GET", "http://example.com/users"),
  );
  ...
  // Test POST
  const postMatch = testRouter.match(
    createRequest("POST", "http://example.com/users"),
  );
  assertExists(postMatch);
  assertEquals(postMatch.handler, postHandler);

  // Test PUT with parameter
  const putMatch = testRouter.match(
    createRequest("PUT", "http://example.com/users/123"),
  );
  ...
  const deleteMatch = testRouter.match(
    createRequest("DELETE", "http://example.com/users/456"),
  );
...
Deno.test("Router extracts single route parameters", () => {
  ...
  const match = testRouter.match(
    createRequest("GET", "http://example.com/users/abc123"),
  );
  assertExists(match);
...
Deno.test("Router handles complex nested parameters", () => {
  ...
  const match = testRouter.match(
    createRequest(
      "GET",
      "http://example.com/api/v1/users/u1/posts/p2/comments/c3",
    ),
  );
  assertExists(match);
...
Deno.test("Router normalizes HTTP methods to uppercase", () => {
  ...
  const upperMatch = testRouter.match(
    createRequest("GET", "http://example.com/test"),
  );
  assertExists(upperMatch);
  assertEquals(upperMatch.handler, handler);

  // Should also match lowercase request
  const lowerMatch = testRouter.match(
    createRequest("get", "http://example.com/test"),
  );
...
Deno.test("Router handles routes without parameters", () => {
  ...
  const match = testRouter.match(
    createRequest("GET", "http://example.com/health"),
  );
  assertExists(match);
  assertEquals(match.handler, handler);
