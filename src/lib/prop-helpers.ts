 ...
 */
export function object<T = Record<string, unknown>>(defaultValue?: T): PropHelper<T> {
  return {
    __propHelper: true,
    type: 'object',
    defaultValue,
 ...
 */
export function extractPropDefinitions(propHelpers: Record<string, PropHelper>): {
  propsTransformer: (attrs: Record<string, string>) => Record<string, unknown>;
  propTypes: Record<string, { type: string; required: boolean; defaultValue?: unknown }>;
} {
  const propTypes: Record<string, { type: string; required: boolean; defaultValue?: unknown }> = {};
