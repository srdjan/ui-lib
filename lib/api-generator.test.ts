...
import { generateClientHx, hx } from "./api-recipes.ts";
...
Deno.test("client hx options override defaults and merge headers", () => {
  const apiMap = {
    create: post("/api/todos", () => new Response("ok")),
  };
  // Use generateClientHx + hx to enrich attributes
  const client = generateClientHx(apiMap);
  const attrString = client.create(hx({
    target: "#inbox",
    swap: "afterbegin",
    headers: { "X-Test": "1" },
  }));
  // Verify attributes are present in the spread string
  assertEquals(attrString.includes('hx-target="#inbox"'), true);
  assertEquals(attrString.includes('hx-swap="afterbegin"'), true);
  // Verify headers string contains our header (HTML-escaped JSON)
  assertEquals(attrString.includes("&quot;X-Test&quot;:&quot;1&quot;"), true);
});
