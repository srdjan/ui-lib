...
Deno.test("JSX runtime detects and renders funcwc components", () => {
  // Clean registry
  resetRegistry();
  ...
  // Test JSX syntax
  const jsxResult = (
    <test-counter count={5} label="My Counter" />
  );
...
Deno.test("JSX runtime handles prop type conversion", () => {
  ...
  const result = (
    <test-props str="hello" num={42} bool={true} />
  );
  
  assertEquals(result.includes("hello-42"), true);
});

Deno.test("JSX runtime falls back to HTML for non-funcwc tags", () => {
