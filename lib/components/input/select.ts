...
export function Select(props: SelectProps): string {
  ...
  const styles = css({
    ...
    select: {
      ...
      ...(size === "sm" && {
        ...
        padding: `0 ${componentTokens.spacing[2]} 0 ${componentTokens.spacing[3]}`,
        fontSize: componentTokens.typography.sizes.sm,
      }),
      ...(size === "md" && {
      ...
      ...(size === "lg" && {
        ...
        padding: `0 ${componentTokens.spacing[3]} 0 ${componentTokens.spacing[4]}`,
    ...
    chevron: {
      ...
      transition: `transform ${componentTokens.animation.duration.fast} ${componentTokens.animation.easing.out}`,
      "&[data-open='true']": {
    ...
    group: {
      padding: `${componentTokens.spacing[2]} ${componentTokens.spacing[3]} ${componentTokens.spacing[1]}`,
      fontSize: componentTokens.typography.sizes.xs,
  ...
  if (selectedOptions.length === 0) {
    displayContent = `<span class="${styles.classMap.placeholder}">${placeholder}</span>`;
  } else if (multiple) {
    const tags = selectedOptions.map(option => 
      `<span class="${styles.classMap.tag}">
        ${option.label}
        ${!disabled && !readOnly ? `<span class="${styles.classMap.tagRemove}" onclick="removeTag('${option.value}')">&times;</span>` : ""}
       </span>`
    ).join("");
    displayContent = `<div class="${styles.classMap.multiValue}">${tags}</div>`;
  } else {
    displayContent = `<span class="${styles.classMap.value}">${selectedOptions[0].label}</span>`;
  }
  
  // Build options list
  const optionsHtml = Object.entries(groupedOptions).map(([group, groupOptions]) => {
    ...
    <div id="${selectId}" class="${styles.classMap.wrapper}">
      ...
      >
        ...
        <div class="${styles.classMap.optionsList}">
          ${optionsHtml || `<div class="${styles.classMap.emptyState}">No options available</div>`}
        </div>
      </div>
