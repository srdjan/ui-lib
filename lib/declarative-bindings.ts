...
import { subscribeToState, publishState, listensFor } from "./reactive-helpers.ts";

/**
 * Process declarative bindings in HTML markup
 * Transforms data-bind-* attributes into working reactive patterns
 */
export function processDeclarativeBindings(html: string, componentName: string): string {
  ...
  processedHtml = processedHtml.replace(
    ...
    (match, stateName) => {
      ...
      return ` data-binding-id="${bindingId}" data-state-topic="${stateName}"` +
             ` hx-on="htmx:load: ${subscribeToState(stateName, `this.textContent = data`)}"`;
    }
  );
  ...
  processedHtml = processedHtml.replace(
    ...
    (match, stateName) => {
      ...
      return ` data-binding-id="${bindingId}" data-state-topic="${stateName}"` +
             ` hx-on="htmx:load: ${subscribeToState(stateName, `this.className = data`)}"`;
    }
  );
  ...
  processedHtml = processedHtml.replace(
    ...
    (match, property, stateName) => {
      ...
      return ` data-binding-id="${bindingId}" data-state-topic="${stateName}"` +
             ` hx-on="htmx:load: ${subscribeToState(stateName, `this.style.${property} = data`)}"`;
    }
  );
  ...
  processedHtml = processedHtml.replace(
    ...
    (match, stateName) => {
      ...
      return ` data-binding-id="${bindingId}" data-state-topic="${stateName}"` +
             ` hx-on="htmx:load: ${subscribeToState(stateName, `this.style.display = data ? '' : 'none'`)}"`;
    }
  );
...
export function hasDeclarativeBindings(html: string): boolean {
  return /data-bind-|data-emit|data-listen|data-show-if|data-hide-if/.test(html);
}

/**
 * Generate unique binding ID for debugging and tracking
 */
function generateBindingId(componentName: string, target: string, type: string): string {
...
export function extractBindingInfo(html: string): Array<{
  ...
  const bindingRegex = /data-binding-id="([^"]+)"(?:[^>]*data-state-topic="([^"]+)")?[^>]*>/g;
  let match;
