...
Deno.test("processDeclarativeBindings - emit binding", () => {
  const html = '<button data-emit="save" data-emit-value=\'{"id": 123}\'>Save</button>';
  const result = processDeclarativeBindings(html, "test-component");
...
Deno.test("processDeclarativeBindings - multiple bindings", () => {
  const html = '<div data-bind-text="title" data-bind-class="theme" data-show-if="visible">Content</div>';
  const result = processDeclarativeBindings(html, "test-component");
...
Deno.test("hasDeclarativeBindings - detection", () => {
  assertEquals(hasDeclarativeBindings('<div data-bind-text="test">Hi</div>'), true);
  assertEquals(hasDeclarativeBindings('<div data-emit="save">Save</div>'), true);
  assertEquals(hasDeclarativeBindings('<div data-listen="load:init()">Listen</div>'), true);
  assertEquals(hasDeclarativeBindings('<div data-show-if="visible">Show</div>'), true);
  assertEquals(hasDeclarativeBindings('<div data-hide-if="hidden">Hide</div>'), true);
  assertEquals(hasDeclarativeBindings('<div class="regular">No bindings</div>'), false);
});
...
Deno.test("validateBindings - validation warnings", () => {
  ...
  const validHtml = '<div data-bind-text="username" data-bind-style="color:theme">Content</div>';
...
Deno.test("processDeclarativeBindings - special characters in handlers", () => {
  const html = '<div data-listen="save:alert(\\"Hello World!\\")">Content</div>';
  const result = processDeclarativeBindings(html, "test-component");
...
Deno.test("processDeclarativeBindings - complex event values", () => {
  const html = '<button data-emit="update" data-emit-value=\'{"user": {"id": 123, "name": "John"}}\'>Update</button>';
  const result = processDeclarativeBindings(html, "test-component");
