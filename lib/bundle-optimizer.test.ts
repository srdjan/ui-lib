...
Deno.test("Tree shaking - Savings calculation", () => {
  ...
  const savings = treeShaking.calculateTreeShakingSavings(
    originalSize,
    optimizedSize,
  );
...
Deno.test("Code splitting - Split benefits calculation", () => {
  ...
  const benefits = codeSplitting.calculateSplitBenefits(
    originalSize,
    criticalSize,
    lazySize,
  );
...
Deno.test("Optimization presets are valid", () => {
  const presets = [
    optimizationPresets.production,
    optimizationPresets.development,
    optimizationPresets.size_optimized,
  ];
  ...
  for (const preset of presets) {
    ...
    assertEquals(
      preset.compressionLevel >= 1 && preset.compressionLevel <= 5,
      true,
    );
