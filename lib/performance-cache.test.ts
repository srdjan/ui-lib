...
Deno.test("Cached render without cache", () => {
  ...
  // First render
  const result1 = cachedRender(
    "test-component-no-cache",
    { prop: "value" },
    renderFn,
  );
  assertEquals(renderCount, 1);
  ...
  const components = [
    {
      ...
      renderFn: (props: Record<string, unknown>) =>
    ...
    },
    {
      ...
      renderFn: (props: Record<string, unknown>) =>
        `<button class="${props.variant}">Button</button>`,
    },
  ];
