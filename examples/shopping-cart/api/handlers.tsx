...
import { h } from "jsx";
import { json, html, error } from "../../../lib/response.ts";
    ...
    if (acceptsHtml) {
      ...
      const productsHtml = result.value.items.map(product => `
        <product-card
          ...
          data-size="${url.searchParams.get("size") || "default"}"
      ...
      return html(`
        <div class="products-grid" data-total="${result.value.total}">
          ${productsHtml}
        </div>
      `);
    } else {
...
export async function getProduct(req: Request, params: { id: string }): Promise<Response> {
  try {
    const repository = getRepository();
    const result = await repository.getProduct(params.id);

    if (!result.ok) {
      return error(result.error.code === "PRODUCT_NOT_FOUND" ? 404 : 400, result.error.message);
    }
...
export async function addToCart(req: Request): Promise<Response> {
  try {
    const sessionId = getSessionId(req);
    const body = await parseJsonBody<AddToCartRequest>(req);
    const repository = getRepository();
...
export async function updateCartItem(req: Request, params: { id: string }): Promise<Response> {
  try {
    const sessionId = getSessionId(req);
...
export async function removeFromCart(req: Request, params: { id: string }): Promise<Response> {
  try {
    const sessionId = getSessionId(req);
    const repository = getRepository();
...
export async function getUser(req: Request, params: { id: string }): Promise<Response> {
  try {
    ...
    if (!result.ok) {
      return error(result.error.code === "USER_NOT_FOUND" ? 404 : 400, result.error.message);
    }
...
export async function getOrder(req: Request, params: { id: string }): Promise<Response> {
  try {
    const repository = getRepository();
    const result = await repository.getOrder(params.id);

    if (!result.ok) {
      return error(result.error.code === "ORDER_NOT_FOUND" ? 404 : 400, result.error.message);
    }
...
export async function getUserOrders(req: Request, params: { userId: string }): Promise<Response> {
  try {
    const repository = getRepository();
