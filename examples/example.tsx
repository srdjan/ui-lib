...
defineComponent("theme-toggle", {
  styles: {
    button:
      `.theme-btn { padding: 0.5rem 1rem; border: 2px solid; border-radius: 6px; cursor: pointer; font-weight: 500; transition: border-color 0.2s ease, box-shadow 0.2s ease; background: none; }`,
    buttonLight:
...
// Counter - Function-style props + API
...
defineComponent("counter", {
  ...
  api: {
    ...
    adjust: patch("/api/counter/adjust", async (req) => {
      ...
      const next = typeof body.value === "number"
        ? body.value
        : (Number(body.current || 0) + Number(body.delta || 0));
  ...
  render: (
    { initialCount = number(0), step = number(1) },
    api: GeneratedApiMap,
    c?: Record<string, string>,
  ) => (
  ...
  render: (
    ...
    const initial = (
      <div>
        ...
        <p>
          This is the content for the {active}{" "}
...
defineComponent("function-style-card", {
  ...
  render: (
    ...
    <div class={c!.container}>
      ...
      <div class={c!.content}>
        <p>
          🎉 <strong>This component uses function-style props!</strong>
        </p>
        <p>
          ✨ No props transformer defined - props auto-generated from render
          function parameters
        </p>
        <p>
          🔢 Count prop: <span class={c!.highlight}>{count}</span>
        </p>
        <p>
          🎯 Enabled prop:{" "}
          <span class={c!.highlight}>{enabled ? "Yes" : "No"}</span>
        </p>
        <p>
          📝 Title prop: <span class={c!.highlight}>"{title}"</span>
        </p>
      </div>
    </div>
  ),
