...
import { cssUtils, modernCSS } from "../lib/modern-css-system.ts";
...
const { styles: modernButtonStyles, report } = css.migrate(
  "button",
  legacyButtonStyles,
);
...
console.log("Production Bundle Stats:", {
  ...
  compressionRatio: `${
    Math.round((1 - prodStats.gzippedSize / prodStats.totalSize) * 100)
  }%`,
});
...
console.log(
  ...
  generateCardHTML({
    ...
    content:
      "This card uses container queries, design tokens, and semantic CSS architecture.",
  }),
);
...
if (typeof window !== "undefined") {
  dev.debug();
  console.log(
    "CSS debugging enabled. Call toggleCSSDebug() in the browser console.",
  );
}
