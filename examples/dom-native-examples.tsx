    ...
    return html`
      <div class="todo ${conditionalClass(isDone, 'done')}" data-id="${props.id}">
        <input 
          ...
          ${spreadAttrs(serverActions?.toggle?.(props.id) || {})}
        />
        <span class="todo-text">${props.text}</span>
        <button class="delete-btn" ${spreadAttrs(serverActions?.delete?.(props.id) || {})}>×</button>
      </div>
  ...
  .view((props) => {
    ...
    return html`
      <div class="accordion ${conditionalClass(isOpen, 'open')}">
        ...
        <div class="accordion-content"><div class="content-inner">${props.content}</div></div>
      </div>
